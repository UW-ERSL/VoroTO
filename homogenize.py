...
class Homogenization:
  def __init__(self, lx: float, ly: float, nelx: int, nely: int,
               phiInDeg: float, matProp: material.MaterialConstants,
               penal: float):
    self.lx = lx
    self.ly = ly
    ...
    self.keLambda, self.keMu, self.feLambda, self.feMu = self.elementMatVec(
                                              self.dx/2, self.dy/2, phiInDeg)
    nodenrs = np.reshape(np.arange(0,(1+self.nely)*(1+self.nelx)),(1+self.nelx,
                                                                   1+self.nely))
